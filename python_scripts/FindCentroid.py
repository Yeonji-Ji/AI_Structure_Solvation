...
def parseData(line):
    if ("CRYST1" in line) or ("REMARK" in line) or ("END" in line) or ("CONNECT" in line):
        # print("Skipping comment line")
        return

    exclude_terms = ['CL', 'NA', 'MG', 'SO4', 'PO4']
    if any(term in line for term in exclude_terms):
        return

    # 실제로 protein (ATOM) 과 ligand (HETATM) 모두 읽기
